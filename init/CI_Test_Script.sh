...
tg_senderror() {
    ...
    tg_sendinfo "<code>Build Throwing Error(s)</code>" \
        ...
        "Logs: https://github.com/karandon24/userbot"
...
lint() {
  ...
  git config --global user.email "kasturiranganskr@gmail.com"
  git config --global user.name "Karandon24"
...
RESULT=`yapf -d -r -p userbot`
  ...
  if [ ! -z "$RESULT" ]; then
      ...
      if [ "$PERMIT_LINT" == "0" ]; then
            ...
            git remote set-url origin https://${GH_USERNAME}:${GH_PERSONAL_TOKEN}@github.com/karandon24/userBot.git
